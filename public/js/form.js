...
const validacedula = async (e) => {
  ...
  try {
    const { data } = await axios.get("https://api.ticketsecuador.ec/pdfqr/api/v1/cedula/" + numero)
...
reporte.addEventListener('click', async function (event) {
  ...
  try {
    const { data, status } = await axios.post('https://api.ticketsecuador.ec/pdfqr/api/v1/registro', _datos)
    if (!data.success) {
