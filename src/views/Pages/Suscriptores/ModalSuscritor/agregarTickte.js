...
export default function ModalTickte({ shows, datosperson, setshows }) {
    ...
    const Endpoitnuevo = async (parms) => {
        try {
            let { data } = await axios.post("https://api.ticketsecuador.ec/ms_login/set_link_external_tickets", parms, {
                headers: {
    ...
    useEffect(() => {
        axios.get("https://api.ticketsecuador.ec/mikroti/api/listApi/GetLocalidad").then(sali => {
            console.log(sali)
            if(sali.status==200){
                setLoacli(sali.data)
            }
