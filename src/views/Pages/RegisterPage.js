...
function RegisterPage() {
  ...
  const Perfils = async () => {
    const { data } = await axios.get("https://api.ticketsecuador.ec/ms_login/api/v1/listas_suscriptor", {
      headers: {
  ...
  const Consulcedula = async () => {
    ...
    try {
      setSpiner("");
      setCedulaapi("d-none");
      const res = await axios.get("https://api.ticketsecuador.ec/ms_login//cedula/" + consulta)
