...
export const Actualisardescripcionevento = async () => {
    try {
        let { data } = axios.get("https://api.t-ickets.com/backend/api/v1/actualisar_descripcion_evento", {
            headers: {
...
const TraerLocalidad = async () => {
    try {
        let { data } = await axios.get("https://api.t-ickets.com/backend/api/v1/listar_localidades/", {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Basic Ym9sZXRlcmlhOmJvbGV0ZXJpYQ=='
            }
...
export const ListaPreciosEvent = async () => {
    ...
    const data = await Promise.all(
        resultado.map(async (e) => {
            const info = await traerprecios("https://api.t-ickets.com/backend/ListaPreciosLocalidades/" + e.codigoEvento)
            e.Precios = info
            return e
        }))
